...
import { enhanceBullet } from '../../utils/aiClient';
...
const ExperienceSection = () => {
  ...
  const handleEnhance = async (expId: string, bulletIndex: number) => {
    ...
    setEnhancing(key);
    try {
      const enhanced = await enhanceBullet(exp.bullets[bulletIndex], exp.jobTitle, exp.company);
      const bullets = [...exp.bullets];
      bullets[bulletIndex] = enhanced;
      updateExp(expId, 'bullets', bullets);
      toast.success('Bullet point enhanced!');
    } catch (err: any) {
      toast.error(err.message || 'Failed to enhance bullet');
    } finally {
      setEnhancing(null);
    }
  };
          ...
          <div>
            ...
            {exp.bullets.map((b, bi) => (
              <div key={bi} className="mb-2 flex gap-2">
                ...
                />
                <button
                  onClick={() => handleEnhance(exp.id, bi)}
                  disabled={enhancing === `${exp.id}-${bi}` || !b}
                  className="shrink-0 rounded-md p-2 text-accent hover:bg-accent/10 disabled:opacity-50"
                  title="Enhance with AI"
                >
                  {enhancing === `${exp.id}-${bi}` ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Sparkles className="h-3.5 w-3.5" />}
                </button>
                <button onClick={() => {
