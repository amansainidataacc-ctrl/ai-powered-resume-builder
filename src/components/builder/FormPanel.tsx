...
const tabs = [
  ...
  { id: 'jd-matcher', label: 'JD Match', icon: Target },
];
  ...
  return (
    <div className="flex flex-col">
      <div className="sticky top-0 z-10 flex gap-1 overflow-x-auto border-b border-border bg-surface-secondary p-2"> 
        {tabs.map(t => (
          <button key={t.id} onClick={() => setActiveTab(t.id)} className={`flex shrink-0 items-center gap-1.5 rounded-md px-3 py-2 text-xs font-medium transition-all ${
            activeTab === t.id ? 'bg-accent text-accent-foreground' : 'text-text-muted hover:bg-surface-hover hover:text-text-secondary'}
          `}>
