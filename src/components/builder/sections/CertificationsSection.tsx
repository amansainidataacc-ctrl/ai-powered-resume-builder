    ...
    <div className="space-y-4">
      ...
      {data.certifications.map(cert => (
        <div key={cert.id} className="relative rounded-card bg-surface-tertiary p-5 space-y-3">
          ...
          <Input label="Organization" value={cert.organization} onChange={v => updateCert(cert.id, 'organization', v)} placeholder="Amazon Web Services" />
          ...
          <Input label="URL" value={cert.url} onChange={v => updateCert(cert.id, 'url', v)} placeholder="https://..." />
        </div>
      ))}
    </div>
  );
