        ...
        <div key={proj.id} className="relative rounded-card bg-surface-tertiary p-5 space-y-3">
          ...
          <div>
            ...
            <textarea value={proj.description} onChange={e => updateProject(proj.id, 'description', e.target.value)}
              ...
              rows={3}
              className="w-full rounded-input border border-input bg-surface-primary px-3 py-2.5 text-sm text-foreground placeholder:text-text-muted focus:border-accent focus:outline-none focus:ring-2 focus:ring-accent/15 resize-none" />
          </div>
          <TechStackInput tags={proj.techStack} onChange={ts => updateProject(proj.id, 'techStack', ts)} />
          <div className="grid grid-cols-2 gap-3">
            <Input label="Live Link" value={proj.liveLink} onChange={v => updateProject(proj.id, 'liveLink', v)} placeholder="https://..." />
            <Input label="GitHub Link" value={proj.githubLink} onChange={v => updateProject(proj.id, 'githubLink', v)} placeholder="https://github.com/..." />
          </div>
        </div>
      ))}
    </div>
...
const TechStackInput = ({ tags, onChange }: { tags: string[]; onChange: (t: string[]) => void }) => {
      ...
      <div className="flex flex-wrap gap-1.5 rounded-input border border-input bg-surface-primary p-2">
        {tags.map((t, i) => (
          <span key={i} className="inline-flex items-center gap-1 rounded-pill bg-accent/15 px-2.5 py-1 text-xs font-medium text-accent">
            {t}
            <button onClick={() => onChange(tags.filter((_, idx) => idx !== i))} className="hover:text-destructive"><X className="h-3 w-3" /></button>
          </span>
        ))}
        <input value={input} onChange={e => setInput(e.target.value)} onKeyDown={handleKey} placeholder="Type + Enter"
